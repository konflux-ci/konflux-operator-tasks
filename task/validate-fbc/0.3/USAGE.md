...
For OCP versions below 4.20, related images must use Docker V2 manifest media types rather than OCI media types, since older OCP versions do not support OCI natively. This check inspects the manifest mediaType of each related image and fails if any use OCI-only types on an incompatible OCP version. The check is skipped for OCP >= 4.20.
